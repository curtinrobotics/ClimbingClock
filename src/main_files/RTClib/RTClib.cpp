...
/*!
    ...
    @attention If year given is 19xx, then this will be treated as 20xx
    @param y Year
    @param m Month
    @param d Day
...
DateTime::DateTime (const __FlashStringHelper* date, const __FlashStringHelper* time) {
    ...
    // Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec
    switch (buff[0]) {
        case 'J': _month = (buff[1] == 'a') ? 1 : ((buff[2] == 'n') ? 6 : 7); break;
        case 'F': _month = 2; break;
        case 'A': _month = buff[2] == 'r' ? 4 : 8; break;
        case 'M': _month = buff[2] == 'r' ? 3 : 5; break;
        case 'S': _month = 9; break;
        case 'O': _month = 10; break;
        case 'N': _month = 11; break;
        case 'D': _month = 12; break;
...
uint32_t DateTime::unixtime(void) const {
  ...
  t = time2long(days, _hour, _minutes, _seconds);
...
DateTime DateTime::operator+(const TimeSpan& span) {
  return DateTime(this->unixtime() + span.totalseconds());
}
...
TimeSpan DateTime::operator-(const DateTime& right) {
  return TimeSpan(this->unixtime() - right.unixtime());
}
  ...
  switch(opt) {
    ...
    default:
        ...
        sprintf(buffer, "%d-%02d-%02dT%02d:%02d:%02d",
                2000+_yearOffset, _month, _day, _hour, _minutes, _seconds);
  }
